...
public class FotoKalender {
	...
	public void makeWhiteXMas(String strOutDir) {
		try {
			...
			sheet.drawText("Wei�e Weihnachten?", sheet.getDefaultTextCol(), "Times New Roman", 0, 0, 0.00, 1, 0.04);
	...
	public void makeOmaSagtZiehDichWarmAn(int month, String strOutDir) {
		try {
			...
			System.out.println("'Oma sagt, warm anziehen' Gesamth�he: " + height);
			...
			yL += h1+10;
			wi = sheet.drawText(" d�nn?�", font1, txtCol, x2, yL);
	...
	public void makeFamilyInZooWider(int month, String strOutDir) {
		try {
			...
			sheet.prepareImage(picW, picH, bckCol);
			int height = sheet.getUsuableHeight();
			System.out.println("'Familie im Zoo' Gesamth�he: " + height);
			int x0 = sheet.getX(0.0);
			int x1 = sheet.getX(0.5)-picW/2;
			int x2 = sheet.getX(0.5)+picW/2;
			int x3 = sheet.getX(1.0);
	...
	public void makeAmbiLightH(int month, String strOutDir) {
		try {
			...
			int height = sheet.getUsuableHeight();
			System.out.println("'AmbiLightH' Gesamtbreite: "+ sheet.getUsuableWidth() +" Gesamth�he: " + height);
			int x0 = sheet.getX(0.0);
			int x1 = sheet.getX(0.5)-picW/2;
			int x2 = sheet.getX(0.5)+picW/2;
			int x3 = sheet.getX(1.0);
	...
	public void makeBerlinerKoennenAlles(int month, String strOutDir) {
		try {
			...
			font1 = new Font("Serif", Font.PLAIN, h1);
			yL += h1+10;
			wi = sheet.drawText("k�nnen alles", font1, Color.BLACK, x0+346, yL);
			System.out.println("k�n all Breite:"+ wi +" "+ (width*3-wi)/2);
			
			h1 = height/4;
			font1 = new Font("Serif", Font.PLAIN, h1);
			yL += h1+10;
			wi = sheet.drawText("au�er", font1, Color.BLACK, x0+661, yL);
			System.out.println("au�er Breite:"+ wi +" "+ (width*3-wi)/2);
	...
	public void make9Windows(int month, String strOutDir) {
		try {
			...
			CalendarSheet sheet = new CalendarSheet(THIS_YEAR, month, this.mDates);
			final int USABLE_WIDTH = 4960;	// maximale Kantenl�nge bei posterxxl.de angeblich 8000 pixel
	...
	public void makeMeinBugDeinBug(int month, String strOutDir) {
		try {
			...
			sheet.drawText("Bugs sind f�r uns alle da",
	...
	public void makeRiversHorseShoeAndMosel(int month, String strOutDir) {
		try {
			...
			// Mosel-U-bei-Sonnenschein.jpg 659x241 pixel R�nder top 7, left 6, right 9, bottom 9
	...
	public void makeDampflok(int nMonth, final String strOutDir) {
		...
		try {
			...
			System.out.println("H�he Dampflokeinzelbilder: "+ h1);
	...
	public void addSpecialEvents() {
		...
		// Die Berlinerinnen und Berliner w�hlen am 20. September 2026 ihr Landesparlament. :
		if (THIS_YEAR == 2026) {
	...
	public void addGermanPublicHolidays() {
		...
		if (easterSunday.get(GregorianCalendar.DAY_OF_MONTH) != 31) {
			// wenn Ostersonntag auf den 31. M�rz f�llt, dann stehen dort 3 Termine: Viktorias Geburtstag, Beginn der Sommerzeit und Ostersonntag.
			// Das ist mir zuviel.
	...
	public void addBavarianPublicHolidays() {
		addCalEventWhenNotFree(PersonalDate.createFixedHoliday("{0,date,EE} Heilige 3 K�nige", 6, 1, THIS_YEAR, PersonalDate.PRIO_PUBLIC_BAVARIAN_HOLIDAY));
		GregorianCalendar easterSunday = EasterSunday.createEasterSunday(THIS_YEAR);
	...
	public void addSundays4Buying() {
		if (this.THIS_YEAR == 2025) {
			...
			sun4buy = new GregorianCalendar(2025, Calendar.JANUARY, 26);  //Anlass ist die Gr�ne Woche in Berlin
			...
			//mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
			//sun4buy = new GregorianCalendar(2019, Calendar.MARCH, 10);  //Internationale TourismusB�rse Berlin (ITB)
			//mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
			//sun4buy = new GregorianCalendar(2020, Calendar.JUNE, 21);  // F�te de la musique
			//mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
			sun4buy = new GregorianCalendar(2025, Calendar.SEPTEMBER, 7); // Anlass ist die Internationale Funkausstellung (IFA)
			mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
			// 01.10.2017: Feier zum Tag der Deutschen Einheit, Er�ffnungswochenende Berlin leuchtet � Lichterfest 2017 
		...
		// Adventssonntage: Veranstaltung von zahlreichen Weihnachtsm�rkten, bereits in addAdvents() gesetzt
		// 7. und 21. Dezember 2025
		if (this.THIS_YEAR > 2025) {
			// TODO: vorl�ufige Einsch�tzungen, denn am 10. September 2025:
			// F�r 2026 gibt es in Berlin noch keine festen Termine f�r verkaufsoffene Sonntage, da diese oft kurzfristig anlassbezogen festgelegt werden.
			GregorianCalendar sun4buy;
			sun4buy = new GregorianCalendar(2025, Calendar.JANUARY, 26);  //Anlass ist die Gr�ne Woche in Berlin
			sun4buy.add(GregorianCalendar.WEEK_OF_YEAR, (int)((this.THIS_YEAR - 2025)*52.18f)); // wohl 52 Wochen sp�ter in 2026
			mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
			sun4buy = new GregorianCalendar(2025, Calendar.SEPTEMBER, 7); // Anlass ist die Internationale Funkausstellung (IFA)
			sun4buy.add(GregorianCalendar.WEEK_OF_YEAR, (int)((this.THIS_YEAR - 2025)*52.18f)); // wohl 52 Wochen sp�ter in 2026
			mDates.addCalEvent(new PersonalDate("", sun4buy, PersonalDate.SUNDAY4BUYING));
	...
	public void addBerlinGrueneWoche() {
		// Die Gr�ne Woche findet vom 16. bis 25. Januar 2026 in den Hallen rund um den Funkturm statt.
		PersonalDate day1;
		IDaysBackground img;
		float fiDay = 16f-THIS_YEAR+2026; // 2027 beginnt die Gr�ne Woche wohl am 15.Januar
	...
	public void addMunichWiesn(int maxFactor) {
		final String imgDirWiesn = "C:\\Users\\MiRoe\\Pictures\\M�nchener Wiesn\\";
